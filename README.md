...
[![Tests](https://img.shields.io/badge/tests-272%20passing-brightgreen.svg)](#development)
...
comb ingest ./company-docs/                         # real files: .docx .pdf .md .txt .csv .json (your data only)
...
Your agent gains 17 governed tools — **read** (`search_brain`, `find_skill`,
...
execute → audit), **prove + learn** (`query_runs`, `declare_intent`,
`list_divergence_candidates`, and `record_outcome` — report what actually happened
after an action lands so the brain re‑weights the records that produced a win), and
**GTM** (`gtm_research_prospect`, `gtm_draft_outreach` — see below).
Semantic (vector) recall + a local/cloud model are an **opt‑in upgrade**
(`LLM_BACKEND=local` / `openai`), never required.

### A GTM persona over the brain (what prompt files can't do)

A static `.md` prompt can't query a corpus bigger than the context window, enforce
access scopes, or refuse when ungrounded. A persona on the brain can. Two MCP tools
turn the brain into a grounded GTM agent:

```
# in your AI tool, over MCP:
gtm_research_prospect {name:"Jane Doe", company:"Acme", role:"CTO"}
  → a dossier built ONLY from cited brain records (your ICP, case studies, wins) — or nothing
gtm_draft_outreach {name:"Jane Doe", company:"Acme", angle:"cut their API test maintenance"}
  → a personalized email grounded in cited records; refuses if there's no grounding (never invents a metric or customer)
```

The point isn't outreach — it's that the persona answers from a governed, scoped,
always-current brain, and **refuses instead of hallucinating**. That's the line a
prompt file can't cross.

### Proof (not claims)

- **272 tests pass** (typecheck + lint + build green); core modules covered.
- **Cite‑or‑refuse, asserted in CI:** the behavioural eval shows the brain *refuses*
  an unseen topic, *answers across sources*, and *hides leadership‑only records from a
  default‑team caller* — scope isolation enforced on every read.
- **No demo data leaks:** a guard test proves a real (`comb install`) brain starts
  empty — it holds only what you ingest.
